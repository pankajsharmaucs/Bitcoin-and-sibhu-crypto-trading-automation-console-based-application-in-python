...
def init():
    # ===========Get current balance
    bal = str(balance())
    logger_shibu("Account Balance USDT:: " + bal)

    # ===========Get current price
    price = get_current_price()
    logger_shibu("Current " + symbol + " Price :: " + str(price))
    ...
    try:
        for i in jsonData:
            ...
            if not QUOTA_LIMIT:
                logger_shibu("Daily quota has exhausted.")
            elif float(bal) < float(BTC_PER_TRADE_QUOTA):
                logger_shibu("Account bal is low.")
            elif buy_status == 0:
                logger_shibu(" Going to Buy order " + str(buy_orderId) + " :: " + str(buy_price))
                isBuyResponse = buy_new(buy_price)
                logger_shibu(" "+str(COIN1)+",Buying price:"+str(buy_price)+",Quantity:"+str(BTC_PER_TRADE_QUOTA/buy_price)+", Order Buy  success :: OrderId:" + str(isBuyResponse['orderId']) + " Status:" + str(
                ...
                remain = total - used
                print(qdata)
                logger_shibu("Daily Quota updated:: remaining quota is " + str(remain))
            ...
            if buy_status == 1:
                ...
                if orderStatus['status'] == "FILLED":
                    ...
                    i.update(newData)
                    logger_shibu("buying order successfully. Set to 2 for order id: " + str(buy_orderId))
                    time.sleep(1)
                elif orderStatus['status'] == "CANCELED":
                    newData = {"buy_status": 0, "buy_orderId": '', "buy_qty": ''}
                    i.update(newData)
                    logger_shibu("buying order reset to 0 for order id: " + str(buy_orderId))
                    time.sleep(1)
                else:
                    logger_shibu(
            ...
            if sell_status == 0 and buy_status == 2:
                qty = i['buy_qty']
                logger_shibu("Going to Sell order " + str(sell_orderId) + " :: " + str(sell_price))
                isSoldResponse = sell_order(sell_price, qty)
                order_id = isSoldResponse['orderId']
                status = isSoldResponse['status']
                logger_shibu(" "+str(COIN1)+",Selling price:"+str(sell_price)+",Quantity:"+str(qty)+"Order Sell success :: "
                                                                                              "OrderId: " + str(
            ...
            if sell_status == 1:
                ...
                if orderStatus['status'] == "FILLED":
                    ...
                    i.update(newData)
                    logger_shibu("Sell is completed for order id: " + str(sell_orderId) + ". Reset buy and sell status to 0")
                    time.sleep(1)
                elif orderStatus['status'] == "CANCELED":
                    ...
                    logger_shibu("Selling status for order id: " + str(
                        sell_orderId) + " is in progress. Current status is " + str(orderStatus['status']))
                    time.sleep(1)

            else:
                logger_shibu("Process Done...")
                time.sleep(1)

        update_json(jsonData)
        logger_shibu("Remaining  Balance USDT:: " + str(balance()))
        print("")
    except ValueError:
        logger_shibu("Something went wrong in execution, please check")
...
def mainloop():
    try:
        c = 0
        while c <= exe_count:
            logger_shibu("Executing buying and selling process...")
            init()
            time.sleep(6)
            c += 1
            if c == exe_count:
        ...
        logger_shibu("Execution Terminated...")
